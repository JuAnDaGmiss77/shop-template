...
const { getProducts, products, createProduct, deleteProduct, getProductById, editProduct } = useProduct()
...
const openModalEdit = () => {
    const modal = window.document.getElementById("modalEdit");
    if (modal) {
        if (modal.classList.contains("hidden")) {
            modal.classList.remove("hidden");
            modal.classList.add("block");
...
const editProductEvent = (idProduct:string) => {
    const productSaved = getProductById(idProduct)

    product.value.id = productSaved.id
    product.value.amount = productSaved.amount
    product.value.image = productSaved.image
    product.value.pricing = productSaved.pricing
    product.value.description = productSaved.description
    product.value.name = productSaved.name
    product.value.amount = productSaved.amount
    product.value.discount = productSaved.discount

    openModalEdit()
}

const onSubmitEdit = () => {
    const productEdit = {...product.value}
    ...
    <div id="modalEdit" tabindex="-1"
        ...
        <div class="relative w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="px-6 py-6 lg:px-8">
                    <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Producto</h3>
                    <form @submit.prevent="onSubmitEdit" class="space-y-6" action="#">
                        <div>
                            <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Imagen Url</label>
                            <input v-model="product.image" type="text" name="email" id="email"
                ...
                <tbody>
                    ...
                    <tr v-for="product in products" :key="product.id"
                        ...
                        <td class="px-6 py-4">
                            ...
                            <a @click="editProductEvent(product.id)" href="#" class="cursor-pointer font-medium text-blue-600 dark:text-red-500 hover:underline">Editar</a>
                        </td>
                    </tr>
